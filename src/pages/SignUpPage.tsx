...
export const SignUpPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setLocalError(err.message || 'Registration failed. Please check your information.');
    } finally {
      ...
      >
        ...
        <div className="text-center space-y-3">
          ...
          <p className="text-sm text-[#B8A89E]">
            Start monitoring freshness with smart IoT today.
          </p>
        </div>
        ...
        <div className="card-solid p-8 space-y-6 shadow-2xl relative border border-[#FF6A00]/25">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <User className="w-4 h-4 text-[#8C7A70] absolute left-3.5 top-1/2 -translate-y-1/2 pointer-events-none" />
                <input
                  type="text"
                  required
                  value={name}
            ...
            {/* Terms checkbox */}
            <div className="flex items-start gap-2 pt-1 text-xs text-[#B8A89E]">
              <input
                type="checkbox"
                checked={termsAccepted}
                onChange={(e) => setTermsAccepted(e.target.checked)}
                className="mt-0.5 w-4 h-4 rounded bg-[#1E140E] border-[#3D261A] text-[#FF6A00] focus:ring-0 cursor-pointer accent-[#FF6A00]"
              />
              <span>
                I agree to the{' '}
                <a href="#terms" className="text-[#FFAA00] hover:text-[#FF6A00] font-bold">
                  Terms of Service
                </a>{' '}
                and{' '}
                <a href="#privacy" className="text-[#FFAA00] hover:text-[#FF6A00] font-bold">
                  Privacy Policy
                </a>
                .
              </span>
          ...
          <div className="text-center pt-2 border-t border-[#3D261A]/60">
            <p className="text-xs text-[#B8A89E]">
              ...
              <Link to="/login" className="text-[#FFAA00] hover:text-[#FF6A00] font-bold">
                Sign In
              </Link>
            </p>
          </div>
