...
export const AdminAlerts: React.FC = () => {
  ...
  return (
    <div className="space-y-5 max-w-5xl mx-auto">
      <div>
        <h1 className="text-2xl font-black text-[#082A52] tracking-tight">System Alerts Console</h1>
        ...
        {(['All', 'Temperature', 'Humidity', 'Air Sensor', 'Offline'] as const).map(f => (
          <button
            ...
            className={`px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all ${
              filter === f
                ? 'bg-[#1267D6] text-white shadow-md'
      ...
      {filteredAlerts.length === 0 ? (
        <div className="glass-card p-8 rounded-3xl text-center space-y-2">
          ...
          <h3 className="text-base font-bold text-[#082A52]">No Active Anomaly Alerts</h3>
            ...
            >
              <div className="flex items-center gap-3.5">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-mono text-sky-700 mt-0.5">Device: {alert.device_id}</p>
                </div>
              </div>
