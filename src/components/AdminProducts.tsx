  ...
  return (
    <div className="space-y-5 max-w-5xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black text-[#1A120D] tracking-tight">Product Profiles</h1>
          <p className="text-xs text-slate-500 font-medium">Define safety thresholds for food categories</p>
        </div>
        <button
          onClick={handleOpenAdd}
          className="px-4 py-2.5 rounded-xl bg-[#FF6A00] text-white font-bold text-xs shadow-md hover:brightness-110 flex items-center gap-1.5 cursor-pointer"
        >
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                <div className="w-10 h-10 rounded-xl bg-[#FF6A00]/15 text-[#FF6A00] font-black text-sm flex items-center justify-center">
                  {prof.name.charAt(0).toUpperCase()}
                </div>
                <div>
                  <h3 className="text-base font-bold text-[#1A120D]">{prof.name}</h3>
                  <span className="text-[10px] font-mono text-slate-400">Profile ID: {prof.id}</span>
                </div>
              </div>
              <button
                onClick={() => handleOpenEdit(prof)}
                className="w-8 h-8 rounded-xl bg-[#FF6A00]/10 text-[#FF6A00] flex items-center justify-center hover:bg-[#FF6A00]/15"
              >
                <Edit3 className="w-4 h-4" />
              </button>
            </div>

            <div className="space-y-1.5 text-xs text-slate-600 bg-[#FF6A00]/10/70 p-3 rounded-2xl">
              <div className="flex justify-between">
                <span>Temp Range:</span>
                <span className="font-bold text-[#1A120D]">{prof.temperature_min}°C to {prof.temperature_max}°C</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-bold text-[#1A120D]">{prof.mq135_threshold} RAW</span>
              </div>
            </div>
          </div>
        ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md">
          <div className="glass-card w-full max-w-md rounded-3xl p-6 bg-white/95 shadow-2xl space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-[#FF6A00]/20">
              <h3 className="text-base font-bold text-[#1A120D]">Product Profile Configuration</h3>
            ...
            <form onSubmit={handleSave} className="space-y-3">
              <div>
                <label className="text-xs font-bold text-[#1A120D] uppercase block mb-1">Category / Product Name</label>
                <input
                  type="text"
                  required
                  value={name}
              ...
              <button
                type="submit"
                className="w-full py-3.5 rounded-xl bg-[#FF6A00] text-white font-bold text-xs shadow-md hover:brightness-110 flex items-center justify-center gap-1.5 cursor-pointer mt-2"
              >
